...
<body>
    ...
    <div id="modalNuevoPrestamo" class="modal modal-prestamo">
        <div class="modal-content">
            ...
            <div class="prestamo-seccion">
                ...
                <div class="input-field">
                    <input id="inputQrTomador" type="text" autocomplete="off" placeholder="Escanee el QR del tomador">
                </div>
            ...
            <div class="prestamo-seccion">
                ...
                <div class="input-field">
                    <input id="inputQrElemento" type="text" autocomplete="off" placeholder="Escanee los elementos">
                </div>
        ...
        <div class="modal-footer">
            ...
            <button id="btnGuardarPrestamo" class="btn btn-guardar btn-accion">
    ...
    <div id="modalDevolucion" class="modal modal-prestamo">
        <div class="modal-content">
            ...
            <div class="input-field">
                <input id="inputQrDevolucion" type="text" autocomplete="off" placeholder="Escanee el QR del elemento">
            </div>
...
<script>
    ...
    async function manejarTomador(e) {
        ...
        const qr_token = input.value.trim();
        if (!qr_token) {
            procesando = false;
            return;
        }
        ...
        try {
            const res = await fetch('/SII-IETSN/api/usuarios/por-hash.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ doc_hash: qr_token })
            });

            const r = await res.json();

            if (!r.success || !r.data) {
                M.toast({ html: r.message || 'Tomador inválido', classes: 'red' });
                return;
            }
            ...
            M.toast({ html: 'Tomador OK', classes: 'green' });
    ...
    async function manejarElemento(e) {
        ...
        if (elementosEscaneados.some(el => el.qr_token === qr_token)) {
            M.toast({ html: 'Elemento repetido', classes: 'orange' });
            input.disabled = false;
            procesando = false;
            return;
        }
        ...
        try {
            ...
            if (!r.success || !r.data) {
                M.toast({ html: r.message || 'Elemento inválido', classes: 'red' });
                return;
            }

            elementosEscaneados.push({ ...r.data, qr_token });

            console.log("Elementos actualizados:", elementosEscaneados);
    ...
    async function manejarDevolucion(e) {
        ...
        try {
            ...
            if (!r.success) {
                M.toast({ html: r.message || 'Error en devolución', classes: 'red' });
                return;
            }
    ...
    async function guardarPrestamo(e) {
        ...
        if (!tomadorActual) {
            M.toast({ html: 'Falta tomador', classes: 'red' });
            return;
        }

        if (!Array.isArray(elementosEscaneados) || elementosEscaneados.length === 0) {
            M.toast({ html: 'Faltan elementos', classes: 'red' });
            return;
        }
    ...
    async function cargarPrestamos() {
        ...
        if (!r.success) {
            M.toast({ html: 'Error al cargar', classes: 'red' });
            return;
        }
